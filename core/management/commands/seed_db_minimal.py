...
class Command(BaseCommand):
    ...
    def _clear(self):
        ...
        from intrants.models import CategorieIntrant, TypeFournisseur
    ...
    def _seed_categories_intrant(self):
        ...
        seeds = [
            ...
            dict(code="POUSSIN",    libelle="كتكوت (دواجن حية)", consommable_en_lot=False, ordre=2),
            dict(code="MEDICAMENT", libelle="دواء / بيطري",      consommable_en_lot=True,  ordre=3),
    ...
    def _seed_categories_depense(self):
        ...
        seeds = [
            dict(code="SALAIRES",    libelle="الرواتب والأجور",          ordre=1, description="رواتب العمال والعمال اليوميين"),
            dict(code="ENERGIE",     libelle="الطاقة (كهرباء / غاز)",    ordre=2, description="فواتير الكهرباء والغاز ووقود التدفئة"),
            dict(code="MAINTENANCE", libelle="الصيانة والإصلاحات",        ordre=3, description="إصلاح المعدات والمباني"),
            dict(code="TRANSPORT",   libelle="النقل والوقود",             ordre=4, description="وقود مركبات التوصيل"),
            dict(code="VETERINAIRE", libelle="المصاريف البيطرية",         ordre=5, description="أتعاب البيطريين (باستثناء الأدوية)"),
            dict(code="FOURNITURES", libelle="اللوازم والتغليف",          ordre=6, description="أكياس، صناديق، تغليف، لوازم مكتبية"),
            dict(code="TAXES",       libelle="الضرائب والرسوم",           ordre=7, description="الضرائب المحلية، الرسوم التجارية، الحقوق المتنوعة"),
            dict(code="DIVERS",      libelle="مصاريف متنوعة",             ordre=8, description="كل مصروف غير مشمول في الفئات أعلاه"),
        ]
        for s in seeds:
    ...
    def _seed_produits_finis(self):
        ...
        specs = [
            dict(designation="دجاج حي (الوزن الكامل)",         type_produit="volaille_vivante", unite="unite", prix=480),
            dict(designation="جثة كاملة منزوعة الأحشاء",       type_produit="carcasse",         unite="kg",    prix=750),
