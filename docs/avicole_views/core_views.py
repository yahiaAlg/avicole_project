...
admin_or_manager_required = user_passes_test(is_admin_or_manager, login_url="core:login")
...
def dashboard(request):
    ...
    try:
        ...
        mouvements_recents = (
            StockMouvement.objects.select_related("intrant", "produit_fini")
...
def company_info_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            logger.info(
                "CompanyInfo updated by user '%s'.", request.user.username
            )
...
def user_password_change(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect("core:user_list" if is_admin(request.user) else "core:dashboard")
        else:
            messages.error(request, "Veuillez corriger les erreurs dans le formulaire.")
    else:
