...
class ProductionLigneInline(admin.TabularInline):
    ...
    extra = 1
    fields = (
        "produit_fini", "quantite", "poids_unitaire_kg",
        "cout_unitaire_estime", "valeur_totale_display", "notes",
    )
...
class ProduitFiniAdmin(ImportExportModelAdmin):
    ...
    list_display = (
        ...
        "prix_vente_defaut", "quantite_en_stock", "actif",
    )
    ...
    fieldsets = (
        ("Catalogue", {
            "fields": ("designation", "type_produit", "unite_mesure", "prix_vente_defaut", "actif"),
        }),
        ("Stock (calculé)", {
...
class ProductionRecordAdmin(ImportExportModelAdmin):
    ...
    list_display = (
        "lot", "date_production", "nombre_oiseaux_abattus",
        "poids_total_kg", "poids_moyen_kg", "statut_badge",
    )
    ...
    fieldsets = (
        ("Production", {
            "fields": (
                "lot", "date_production",
                "nombre_oiseaux_abattus", "poids_total_kg", "poids_moyen_kg",
                "statut",
            ),
        }),
        ("Notes", {"fields": ("notes",), "classes": ("collapse",)}),
...
class ProductionLigneAdmin(ImportExportModelAdmin):
    ...
    list_display = (
        "production", "produit_fini", "quantite",
        "poids_unitaire_kg", "cout_unitaire_estime", "valeur_totale_display",
    )
