...
class BLFournisseurLigneInline(admin.TabularInline):
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        if obj and obj.est_verrouille:
            return ("intrant", "quantite", "prix_unitaire", "montant_total_display", "notes")
...
class BLFournisseurAdmin(ImportExportModelAdmin):
    ...
    list_display = (
        "reference", "fournisseur", "date_bl",
        "statut_badge", "montant_total_dzd", "a_piece_jointe", "created_at",
    )
    ...
    fieldsets = (
        ("Entête", {
            "fields": (
                "reference", "fournisseur", "date_bl",
                "reference_fournisseur", "statut", "est_verrouille",
            ),
        }),
        ("Pièce jointe & notes", {
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        if obj and obj.est_verrouille:
            ...
            base += ["reference", "fournisseur", "date_bl", "reference_fournisseur", "statut"]
        return base
...
class FactureFournisseurAdmin(ImportExportModelAdmin):
    ...
    list_display = (
        "reference", "fournisseur", "date_facture", "type_facture",
        "montant_total_dzd", "montant_regle_dzd", "reste_a_payer_dzd",
    ...
    readonly_fields = (
        "montant_total", "montant_regle", "reste_a_payer",
    ...
    fieldsets = (
        ("Facture", {
            "fields": (
                "reference", "fournisseur", "date_facture",
                "date_echeance", "type_facture",
            ),
        }),
        ("BL inclus", {
            "fields": ("bls",),
...
class ReglementFournisseurAdmin(ImportExportModelAdmin):
    ...
    list_display = (
        "fournisseur", "date_reglement",
        "montant_dzd", "mode_paiement", "reference_paiement", "created_at",
    )
    ...
    fieldsets = (
        ("Règlement", {
            "fields": (
                "fournisseur", "date_reglement", "montant",
...
class AcompteFournisseurAdmin(ImportExportModelAdmin):
    ...
    list_display = (
        "fournisseur", "montant_dzd", "date", "utilise", "created_at",
    )
