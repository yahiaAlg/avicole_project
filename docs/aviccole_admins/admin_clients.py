...
class BLClientLigneInline(admin.TabularInline):
    model = BLClientLigne
    extra = 1
    fields = ("produit_fini", "quantite", "prix_unitaire", "montant_total_display", "notes")
    ...
    def get_readonly_fields(self, request, obj=None):
        if obj and obj.est_verrouille:
            return ("produit_fini", "quantite", "prix_unitaire", "montant_total_display", "notes")
...
class ClientAdmin(admin.ModelAdmin):
    list_display = (
        "nom", "type_client", "wilaya", "telephone",
        "actif", "creance_globale_dzd", "plafond_depasse",
    )
    list_filter = ("actif", "type_client", "wilaya")
    search_fields = ("nom", "nif", "rc", "telephone", "email")
    readonly_fields = ("created_at", "updated_at", "creance_globale_dzd", "plafond_depasse")
    ...
    fieldsets = (
        ...
        ("Coordonnées", {
            "fields": ("adresse", "wilaya", "telephone", "telephone_2", "email", "contact_nom"),
        }),
        ("Identifiants légaux", {
            "fields": ("nif", "rc"),
            "classes": ("collapse",),
    ...
    def creance_globale_dzd(self, obj):
        ...
        if val > 0:
            return format_html('<span style="color:red;font-weight:bold">{} DZD</span>', f"{val:,.2f}")
        return "0,00 DZD"
...
class BLClientAdmin(admin.ModelAdmin):
    list_display = (
        "reference", "client", "date_bl",
        "statut_badge", "montant_total_dzd", "created_at",
    )
    list_filter = ("statut", "client", "date_bl")
    search_fields = ("reference", "client__nom", "signe_par")
    date_hierarchy = "date_bl"
    readonly_fields = ("created_at", "updated_at", "montant_total_dzd", "est_verrouille")
...
class BLClientLigneAdmin(admin.ModelAdmin):
    list_display = ("bl", "produit_fini", "quantite", "prix_unitaire", "montant_total_dzd")
...
class FactureClientAdmin(admin.ModelAdmin):
    list_display = (
        "reference", "client", "date_facture",
        "montant_ttc_dzd", "montant_regle_dzd", "reste_a_payer_dzd",
        "statut_badge", "en_retard",
    )
    ...
    fieldsets = (
        ...
        ("Finances (calculé automatiquement)", {
            "fields": (
                "montant_ht", "montant_tva", "montant_ttc",
                "montant_regle", "reste_a_payer", "statut",
            ),
        }),
        ("Notes", {"fields": ("notes",), "classes": ("collapse",)}),
        ("Horodatage", {
...
class PaiementClientAdmin(admin.ModelAdmin):
    list_display = (
        "client", "date_paiement", "montant_dzd",
        "montant_alloue_dzd", "solde_non_alloue_dzd",
    ...
    def solde_non_alloue_dzd(self, obj):
        ...
        if val > 0:
            return format_html('<span style="color:orange">{} DZD</span>', f"{val:,.2f}")
        return "0,00 DZD"
