...
class LotElevageForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["fournisseur_poussins"].queryset = Fournisseur.objects.filter(actif=True).order_by("nom")
    ...
    def clean_nombre_poussins_initial(self):
        ...
        if val < 1:
            raise ValidationError("Le nombre de poussins initial doit être supérieur à 0.")
        return val
...
class LotFermetureForm(forms.Form):
    ...
    def clean_date_fermeture(self):
        ...
        if date > datetime.date.today():
            raise ValidationError("La date de fermeture ne peut pas être dans le futur.")
        return date
...
class MortaliteForm(forms.ModelForm):
    ...
    def __init__(self, *args, lot=None, **kwargs):
        ...
        """
        super().__init__(*args, **kwargs)
        self.fields["lot"].queryset = LotElevage.objects.filter(statut=LotElevage.STATUT_OUVERT)
        if lot:
