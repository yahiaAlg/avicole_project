...
class ProductionRecordForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if date_prod and date_prod > datetime.date.today():
            self.add_error("date_production", "La date de production ne peut pas être dans le futur.")
...
class ProductionLigneForm(forms.ModelForm):
    class Meta:
        ...
        fields = ["produit_fini", "quantite", "poids_unitaire_kg", "cout_unitaire_estime", "notes"]
        widgets = {
            "quantite": forms.NumberInput(attrs={"step": "0.001", "min": "0.001"}),
            "poids_unitaire_kg": forms.NumberInput(attrs={"step": "0.001", "min": "0"}),
            "cout_unitaire_estime": forms.NumberInput(attrs={"step": "0.0001", "min": "0"}),
