...
def calculer_ic(total_aliment_kg: Decimal, poids_total_produit_kg: Decimal) -> Decimal | None:
    """
...
def get_lot_summary(lot) -> dict:
    ...
    # --- Consommation (feed) --------------------------------------------
    conso_aliment_qs = lot.consommations.filter(
        intrant__categorie__code="ALIMENT"
    )
    total_aliment_kg = (
        conso_aliment_qs.aggregate(total=Sum("quantite"))["total"] or Decimal("0")
    )
    ...
    poids_total_produit_kg = (
        productions.aggregate(total=Sum("poids_total_kg"))["total"] or Decimal("0")
    )
    ...
    cout_total_depenses = (
        depenses.aggregate(total=Sum("montant"))["total"] or Decimal("0")
    )
