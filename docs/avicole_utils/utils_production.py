...
def allouer_cout_production(production_record) -> None:
    ...
    cout_depenses = (
        lot.depenses.aggregate(total=Sum("montant"))["total"] or Decimal("0")
    )
...
def get_rendement_abattage(production_record) -> Decimal | None:
    ...
    """
    if not production_record.poids_moyen_kg or production_record.nombre_oiseaux_abattus == 0:
        return None
    ...
    poids_vif_total = (
        Decimal(str(production_record.nombre_oiseaux_abattus))
        * Decimal(str(production_record.poids_moyen_kg))
    )
...
def get_production_dashboard(date_debut=None, date_fin=None) -> list[dict]:
    ...
    rows: dict[int, dict] = defaultdict(lambda: {
        "lot": None,
    ...
    for record in qs:
        ...
        row["poids_somme_ponderee"] += (
            Decimal(str(record.poids_moyen_kg))
            * Decimal(str(record.nombre_oiseaux_abattus))
        )
        if row["date_production"] is None or record.date_production > row["date_production"]:
    ...
    for row in rows.values():
        ...
        poids_moyen = (
            row["poids_somme_ponderee"] / Decimal(str(nb))
            if nb > 0 else Decimal("0")
        )
        result.append({
            "lot": row["lot"],
